...
(*            let () = prerr_endline @@ "nonZero BSDF" in *)
            let weight =
              ...
              then
                ...
                self#mis2 (factor.oDirectPdfW *. lightPickProb) (factor.oDirectPdfW *. contProb)
              else 1. in
...
(*               Printf.fprintf stderr "isect : %a\n" Isect.dump isect; *)
               ...
               then
                 ...
                 then self#accLightNoReflect pathLength ray isect bsdf lastSpecular
                   ...
                   else ((* prerr_endline @@ "maxPathLength" ^ string_of_float (Bsdf.continuationProb bsdf); *)
                     color)
               else ((*prerr_endline @@ "invalid bsdf ("^(string_of_int pathLength) ^")";*)
                 color)
          in
